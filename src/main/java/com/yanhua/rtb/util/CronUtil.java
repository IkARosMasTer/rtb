...
public class CronUtil {
    ...
    public static String convertCronExpression(Date startDate, Date endDate, String[] weeks) {
        return convertSeconds() + " " + convertMinutes(startDate) + " " +
                convertHours(startDate, endDate) + " " + convertDay(startDate, endDate) + " " +
                convertMonth(startDate, endDate) + " " + convertWeek(weeks);
    }
    ...
    public static Date getStartDate(Date startDate) throws ParseException {
        String yyyyMMdd = DateUtils.date2String(startDate, "yyyyMMdd");
        Calendar startCal = Calendar.getInstance();
        startCal.setTime(startDate);
        int startHour = startCal.get(Calendar.HOUR_OF_DAY);
        // 12小时制小时（calendar.HOUR，calendar.HOUR_OF_DAY 24小时）
        ...
        return DateUtils.string2Date(yyyyMMdd + startTime + "00", "yyyyMMddHHmmss");
    }
    ...
    public static String convertHours(Date startDay, Date endDay) {
        ...
        int startHour = startCal.get(Calendar.HOUR_OF_DAY);
        // 小时（calendar.HOUR)12小时制，calendar.HOUR_OF_DAY 24小时）
        Calendar endCal = Calendar.getInstance();
...
//        List<String> strings = Arrays.stream(dayOfWeeks).collect(Collectors.toList());
        return StringUtils.join(dayOfWeeks,",");
